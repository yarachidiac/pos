...
const Section = ({addTitle, setAddTitle, companyName }) => {
  ...
  const handleEditClick = (title) => {
    setAddTitle(title)
    console.log("Edit clicked for:", sectionId);
    setIsDialogOpen(true);
  ...
  const onAdd = async (sectionInfo) => {
    try {
      if (addTitle === "Add Section") {
        ...
        const apiUrl = `http://192.168.16.113:8000/updateSections/${companyName}/${sectionId}`;
              ...
              >
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={(event) => {
                      event.stopPropagation(); // Prevent click event from propagating to the Box
                      setSectionId(section.SectionNo);
